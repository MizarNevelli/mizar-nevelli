...
const SITE_URL = process.env.VITE_SITE_URL ?? "https://mizarnevelli.js.org";
  ...
  const routes: RouteMeta[] = [
    {
      ...
      title: "Mizar",
      description:
        "Interactive JavaScript explainers built by a developer who codes on the road.",
    },
    {
      path: "/about",
      title: "About",
      description:
        "The story behind the name — a developer, seven countries, eight years of JavaScript.",
    },
    {
      path: "/blog",
      title: "Blog",
      description:
        "Field notes from the road. Travel dispatches from a remote JavaScript engineer.",
    },
    {
      path: "/contact",
      title: "Contact",
      description:
        "Get in touch about projects, freelance work, or collaborations.",
    },
    {
      path: "/event-loop",
      title: "Event Loop",
      description:
        "A visual walkthrough of the JavaScript event loop — call stack, microtasks, and task queue.",
    },
    {
      path: "/event-bubbling",
      title: "Event Bubbling",
      description:
        "See how DOM events propagate through the tree, step by step.",
    },
    {
      path: "/closures",
      title: "Closures",
...
function buildMetaBlock(route: RouteMeta): string {
  const fullTitle =
    route.title === "Mizar" ? "Mizar" : `${route.title} · Mizar`;
    ...
    async closeBundle() {
      ...
      for (const file of images) {
        ...
        if (ext === ".png")
          ...
          buf = await img.webp({ quality: 60 }).toBuffer();
        else
          buf = await img
      ...
      console.warn(
        `[compress-blog-images] ${images.length} image(s), saved ${kb} KB`
      );
    },
  };
}
...
function staticMeta(): Plugin {
  return {
    ...
    closeBundle() {
      ...
      console.warn(`[static-meta] wrote ${routes.length} HTML files`);
    },
  };
}
